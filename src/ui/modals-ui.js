...
import {
    ...
    // Highlights
    highlightModal, highlightForm, highlightPlanIdInput, highlightChapterName, highlightVersesInput,
    // Meus Grifos (NOVO)
    myHighlightsModal, myHighlightsList
...
const allModals = [
    recalculateModal, statsModal, historyModal, syncModal, bibleExplorerModal, versionModal, highlightModal, myHighlightsModal
];
...
/**
 * Renderiza o histórico de leitura do usuário focado unicamente nos dias e capítulos.
 * Grifos foram movidos para uma visualização exclusiva (displayMyHighlights) para melhor UX.
 */
export function displayHistory(plan) {
    historyListDiv.innerHTML = '';
    hideError('history-modal');

    const log = plan.readLog || {};
    ...
    sortedDates.forEach(dateStr => {
        ...
        let chaptersText = chaptersRead.length > 0 ? chaptersRead.join(', ') : 'Nenhum capítulo registrado.';
...
/**
 * [NOVO] Renderiza o painel exclusivo para visualizar os grifos independentemente da conclusão do dia.
 */
export function displayMyHighlights(plan) {
    if (!myHighlightsList) {
        console.error("Erro: Contêiner 'my-highlights-list' não encontrado no DOM.");
        return; 
    }

    myHighlightsList.innerHTML = '';
    const highlights = plan.versesToHighlight || {};
    let hasVisibleHighlights = false;

    if (Object.keys(highlights).length === 0) {
        myHighlightsList.innerHTML = '<p style="text-align: center; color: var(--text-color-muted);">Você ainda não destacou nenhum versículo neste plano.</p>';
        return;
    }

    for (const [chapter, versesArray] of Object.entries(highlights)) {
        // Ignora o capítulo inteiro se não houver versículos pendentes
        const hasPending = versesArray.some(v => !v.done);
        if (!hasPending) continue;

        hasVisibleHighlights = true;
        let html = `<div class="highlight-chapter-group">
                      <h3 class="highlight-chapter-title">${chapter}</h3>
                      <div class="history-verses-list">`;
        
        versesArray.forEach((v, idx) => {
            // Se o versículo já foi grifado na Bíblia física, não o renderiza
            if (v.done) return;

            html += `<div class="history-verse-item">
                        <input type="checkbox" class="verse-highlight-check" data-plan-id="${plan.id}" data-chapter="${chapter}" data-verse-index="${idx}">
                        <span class="neon-indicator">🖌️</span> Versículo(s) ${v.verses}
                     </div>`;
        });
        
        html += `</div></div>`;
        myHighlightsList.innerHTML += html;
    }

    // Feedback visual caso tudo tenha sido limpo
    if (!hasVisibleHighlights) {
         myHighlightsList.innerHTML = '<p style="text-align: center; color: var(--success-color); font-weight: bold;">🎉 Todos os seus grifos já foram passados para a Bíblia física!</p>';
    }
}
...
export function init(callbacks) {
    ...
    // Intercepta o salvamento do formulário de versículos
    if (highlightForm) {
        highlightForm.addEventListener('submit', (e) => {
            e.preventDefault(); // Impede que a página recarregue ao clicar em "Salvar"
            
            const planId = highlightPlanIdInput.value;
            const chapterName = highlightChapterName.textContent;
            const versesStr = highlightVersesInput.value;
            
            state.callbacks.onSaveHighlight?.(planId, chapterName, versesStr);
        });
    }
    ...
    if (variablePaceConfig && radioOptions.length > 0) {
        radioOptions.forEach(radio => {
            radio.addEventListener('change', (e) => {
                if (e.target.value === 'variable_pace') {
                    variablePaceConfig.style.display = 'block';
                } else {
                    variablePaceConfig.style.display = 'none';
                }
            });
        });
