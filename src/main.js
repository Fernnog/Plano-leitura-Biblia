...
async function handleAuthStateChange(user) {
    ...
    if (user) {
        ...
        renderAllPlanCards();
        // ATUALIZADO: Passa o activePlanId para o paginador para garantir o destaque correto.
...
async function handlePlanSubmit(formData, planId) {
    ...
    try {
        ...
        renderAllPlanCards();
        sidePanelsUI.render(appState.userPlans, appState.activePlanId, handleSwitchPlan);
        // ATUALIZADO: Garante que o paginador reflita a nova lista de planos e o plano ativo.
        floatingNavigatorUI.render(appState.userPlans, appState.activePlanId);
...
async function handleCompleteDay(planId) {
    ...
    try {
        ...
        renderAllPlanCards();
        // ATUALIZADO: Garante que os painéis laterais e o paginador sejam re-renderizados com as novas informações.
        sidePanelsUI.render(appState.userPlans, appState.activePlanId, handleSwitchPlan);
        floatingNavigatorUI.render(appState.userPlans, appState.activePlanId);
...
async function handleDeletePlan(planId) {
    ...
    if (confirm(`Tem certeza que deseja excluir o plano "${planToDelete.name}"? Esta ação não pode ser desfeita.`)) {
        try {
            ...
            renderAllPlanCards();
            sidePanelsUI.render(appState.userPlans, appState.activePlanId, handleSwitchPlan);
            // ATUALIZADO: Garante que o paginador reflita a remoção do plano.
            floatingNavigatorUI.render(appState.userPlans, appState.activePlanId);
...
async function handleRecalculate(option, newPaceValue, planId) {
    ...
    try {
        ...
        await loadInitialUserData(appState.currentUser);
        renderAllPlanCards();
        // ATUALIZADO: Garante que os painéis e o paginador sejam re-renderizados após recálculo.
        sidePanelsUI.render(appState.userPlans, appState.activePlanId, handleSwitchPlan);
        floatingNavigatorUI.render(appState.userPlans, appState.activePlanId);
...
async function handleCreateFavoritePlanSet() {
    try {
        ...
        renderAllPlanCards();
        sidePanelsUI.render(appState.userPlans, appState.activePlanId, handleSwitchPlan);
        // ATUALIZADO: Garante que o paginador reflita a criação dos novos planos.
        floatingNavigatorUI.render(appState.userPlans, appState.activePlanId);
    } catch (error) {
