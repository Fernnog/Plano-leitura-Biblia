...
// NOVO: Importação do módulo de construção de planos (próximo passo será criá-lo)
...
// REFATORADO: Função de submissão de plano agora delega a lógica de construção.
async function handlePlanSubmit(formData, planId) {
    planCreationUI.showLoading();
    try {
        if (planId) {
            // --- Lógica de EDIÇÃO (permanece simples) ---
            ...
            // --- Lógica de CRIAÇÃO (agora abstraída) ---
            // 1. Chama o novo módulo builder para construir o objeto do plano.
            //    Ele encapsula toda a complexidade de cálculos de capítulos e datas.
            const newPlanData = buildPlanFromFormData(formData);

            // 2. O orquestrador apenas pega o resultado e salva.
        ...
        // --- Orquestração da UI (comum a ambos os casos) ---
        planCreationUI.hide();
        planCreationActionsSection.style.display = 'flex';
