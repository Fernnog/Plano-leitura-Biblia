...
async function markAsRead() { if (!currentReadingPlan || !currentUser || !activePlanId || (markAsReadButton && markAsReadButton.disabled)) return; const userId = currentUser.uid; const { plan, currentDay, allowedDays } = currentReadingPlan; const totalCalendarDays = Object.keys(plan || {}).length; if (currentDay > 0 && currentDay <= totalCalendarDays) { const currentDayStr = currentDay.toString(); const chaptersJustRead = plan[currentDayStr] || []; const currentDateString = getCurrentUTCDateString(); const currentWeekId = getUTCWeekId(); let nextCalendarDay = currentDay + 1; const validAllowedDays = Array.isArray(allowedDays) && allowedDays.length > 0 ? allowedDays : [0, 1, 2, 3, 4, 5, 6]; if (validAllowedDays.length < 7) { const startDateForSim = new Date(Date.UTC(2024, 0, 1)); // Use the same fixed start date as distributePlan while (nextCalendarDay <= totalCalendarDays * 2) { // Search limit const simDate = new Date(startDateForSim); simDate.setUTCDate(startDateForSim.getUTCDate() + (nextCalendarDay - 1)); // Day index is 1-based const dayOfWeek = simDate.getUTCDay(); if (validAllowedDays.includes(dayOfWeek)) { break; } nextCalendarDay++; } } let updatedWeeklyData = JSON.parse(JSON.stringify(currentWeeklyInteractions || { weekId: null, interactions: {} })); if (updatedWeeklyData.weekId !== currentWeekId) updatedWeeklyData = { weekId: currentWeekId, interactions: {} }; if (!updatedWeeklyData.interactions) updatedWeeklyData.interactions = {}; updatedWeeklyData.interactions[currentDateString] = true; const logEntry = (chaptersJustRead.length > 0) ? { date: currentDateString, chapters: chaptersJustRead } : null; const success = await updateProgressInFirestore(userId, activePlanId, nextCalendarDay, updatedWeeklyData, logEntry); if (success) { loadDailyReadingUI(); updateWeeklyTrackerUI(); if (nextCalendarDay > totalCalendarDays) { setTimeout(() => alert(`Você concluiu o plano "${currentReadingPlan.name || ''}"! Parabéns!`), 100); } } } else { console.warn("Tentativa de marcar como lido quando plano já concluído ou inválido."); } }
function handleDeleteSpecificPlan(planIdToDelete) { // Wrapper function for confirm
    ...
    if (confirm(`Tem certeza que deseja excluir o plano "${planName}" permanentemente? Todo o progresso será perdido.`)) {
        deletePlanFromFirestore(userId, planIdToDelete) // Call the actual delete function
            .then(success => {
                if (success) {
