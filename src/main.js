...
// MODIFICADO: Importa a função addUTCDays para o cálculo da previsão
...
// MODIFICADO: Função de recalcular agora armazena histórico
...
// MODIFICADO: Função de estatísticas com cálculos avançados e dados para o gráfico
function handleShowStats(planId) {
    const plan = appState.userPlans.find(p => p.id === planId);
    if (!plan) return;

    // --- LÓGICA DE CÁLCULO EXISTENTE E NOVA ---
    ...
    // Objeto final com todos os dados para a UI
    const stats = {
