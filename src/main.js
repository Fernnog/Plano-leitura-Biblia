...
async function handleSaveHighlight(planId, chapterName, versesStr) {
    ...
    try {
        ...
        // Simulação de chamada de serviço para persistir no banco de dados.
        // Necessário criar essa função no planService posteriormente.
        await planService.saveVerseHighlight(appState.currentUser.uid, planId, chapterName, versesStr);
        
        // Atualização em memória para refletir na interface imediatamente
        const plan = appState.userPlans.find(p => p.id === planId);
        if (plan) {
