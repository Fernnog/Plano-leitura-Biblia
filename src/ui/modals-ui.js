...
const allModals = [
    recalculateModal, statsModal, historyModal, syncModal, bibleExplorerModal, versionModal, highlightModal
];
...
/**
 * MODIFICADO: Agora recebe o objeto `plan` completo ao invés de apenas `readLog`.
 * Renderiza o histórico de leitura e os versículos a serem grifados (Highlights).
 */
export function displayHistory(plan) {
    ...
    const highlights = plan.versesToHighlight || {}; // Nova propriedade no banco de dados
    ...
    sortedDates.forEach(dateStr => {
        ...
        // Verifica se há versículos para grifar nestes capítulos lidos no dia
        let highlightsHTML = '';
        chaptersRead.forEach(cap => {
            if (highlights[cap] && highlights[cap].length > 0) {
                highlightsHTML += `<div class="history-verses-list"><strong>${cap}:</strong>`;
                highlights[cap].forEach((v, idx) => {
                    const isDone = v.done ? 'checked' : '';
                    const doneClass = v.done ? 'done' : '';
                    highlightsHTML += `
                        <div class="history-verse-item ${doneClass}">
                            <input type="checkbox" class="verse-highlight-check" data-plan-id="${plan.id}" data-chapter="${cap}" data-verse-index="${idx}" ${isDone}>
...
export function init(callbacks) {
    ...
    // NOVO: Intercepta o salvamento do formulário de versículos
    if (highlightForm) {
