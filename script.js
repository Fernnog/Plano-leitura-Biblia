...
let currentReadingPlan = null; // Deve conter { id, name, plan, currentDay, startDate, endDate, allowedDays, chaptersList, ... }
let userPlansList = []; // Lista de todos os planos do usuário { id, name, currentDay, totalDays, startDate, endDate, allowedDays, plan, chaptersList, ... }
...
const startDateInput = document.getElementById('start-date-input'); // Mantido, mas lógica pode precisar de ajuste
const endDateInput = document.getElementById('end-date-input');   // Mantido, mas lógica pode precisar de ajuste
...
const upcomingReadingsSection = document.getElementById('upcoming-readings'); // *** NOVO Elemento Section ***
const upcomingReadingsListDiv = document.getElementById('upcoming-readings-list'); // *** NOVO Elemento Div dentro da section ***
const upcomingReadingsLoadingDiv = document.getElementById('upcoming-readings-loading'); // *** NOVO Loading Indicator ***
...
function distributeChaptersOverReadingDays(chaptersToRead, totalReadingDays) {
    ...
    if (totalChapters === 0 || isNaN(totalReadingDays) || totalReadingDays <= 0) {
        ...
        // Retorna um mapa com chaves para os dias, mas listas vazias
    ...
    for (let dayNumber = 1; dayNumber <= totalReadingDays; dayNumber++) {
        ...
        const endSliceIndex = Math.min(chapterIndex + chaptersForThisDayCount, totalChapters); // Garante não exceder
    ...
    // Garante que todos os capítulos foram distribuídos (importante para cálculo da data final)
    ...
    //console.log(`Capítulos distribuídos em ${totalReadingDays} dias de leitura.`);
    return planMap;
}
...
function updateBookSuggestions() {
    ...
    // Só mostra sugestões se houver texto relevante e não for só número/hífen
    if (relevantText && !/^\d+\s*(-?\s*\d*)?$/.test(relevantText)) {
        ...
        matchingBooks.slice(0, limit).forEach(book => {
            ...
            // Sugere o nome completo do livro para facilitar
            const prefix = lastCommaIndex >= 0 ? currentText.substring(0, lastCommaIndex + 1) + ' ' : '';
            option.value = prefix + book + ' '; // Adiciona espaço para facilitar digitação do cap.
            option.label = book; // O que aparece na lista
...
function updateWeeklyTrackerUI() {
    ...
    // Verifica se os dados locais SÃO da semana atual E se existem interações
    ...
    const weekStartDate = getUTCWeekStartDate(); // Obtém o domingo da semana atual em UTC

    dayIndicatorElements.forEach(el => {
        const dayIndex = parseInt(el.dataset.day, 10); // 0 para Dom, 1 para Seg...
        // Calcula a data UTC para este dia específico da semana
        const dateForThisDay = new Date(weekStartDate);
        dateForThisDay.setUTCDate(weekStartDate.getUTCDate() + dayIndex);
        const dateString = dateForThisDay.toISOString().split('T')[0]; // Formato 'YYYY-MM-DD'

        // Ativa o marcador se os dados da semana são válidos E existe uma entrada para esta data
...
function updateUIBasedOnAuthState(user) {
    ...
    if (user) {
        ...
        // Resetar todo o estado da aplicação
        userInfo = null;
        activePlanId = null;
        ...
        if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'none'; // Esconde upcoming
        ...
        clearHistoryUI();
        clearStatsUI();
        clearUpcomingReadingsUI(); // Limpa upcoming
...
function resetFormFields() {
    ...
    if (daysInput) daysInput.value = "30"; // Duração em dias calendário
    if (startDateInput) startDateInput.value = ''; // Data de Início (calendário)
    if (endDateInput) endDateInput.value = '';   // Data Final (calendário)
    if (chaptersPerDayInput) chaptersPerDayInput.value = '3'; // Caps/dia *de leitura*
    ...
    togglePlanCreationOptions(); // Reajusta a visibilidade das opções
}
...
function updateProgressBarUI() {
    ...
    const totalReadingDaysInPlan = Object.keys(plan || {}).length; // Total de dias com leitura designada
    ...
    if (totalReadingDaysInPlan > 0 && startDate && endDate) {
        ...
        // Calcula a porcentagem baseada nos dias de LEITURA
        ...
        if (isCompleted) {
            ...
            // Mostra Dia X de Y (dias de leitura) e as datas
...
function populatePlanSelector() {
    ...
    if (userPlansList.length === 0) {
        ...
        planSelectorContainer.style.display = 'flex'; // Mostra mesmo vazio
        return;
    }
    ...
    userPlansList.forEach(plan => {
        ...
        // Inclui as datas no nome do plano no seletor para contexto
...
function populateManagePlansModal() {
    ...
    userPlansList.forEach(plan => {
        ...
        const dateInfo = (plan.startDate && plan.endDate)
            ...
            : '<small style="display: block; color: red; font-size: 0.8em;">Datas não definidas</small>'; // Indica se faltam datas
        item.innerHTML = `
            <div>
    ...
    // Re-atribui listeners após popular
    ...
    planListDiv.querySelectorAll('.delete-plan-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            handleDeleteSpecificPlan(planIdToDelete); // Chama a função wrapper
        });
    });
}
...
function clearStatsUI() {
    ...
    if(statsContentDiv) statsContentDiv.style.display = 'block'; // Mostra área de conteúdo
    if(statsErrorDiv) showErrorMessage(statsErrorDiv, ''); // Limpa erros
}
...
async function fetchUserPlansList(userId) {
    ...
    userPlansList = []; // Limpa a lista local antes de buscar
    try {
        const querySnapshot = await getDocs(q);
        querySnapshot.forEach((docSnap) => {
            // Armazena dados completos, incluindo id e datas se existirem
...
async function loadActivePlanData(userId, planId) {
    if (!userId || !planId) {
        ...
        readingPlanSection.style.display = 'none';
        if(progressBarContainer) progressBarContainer.style.display = 'none';
        planCreationSection.style.display = userPlansList.length === 0 ? 'block' : 'none'; // Mostra criação se não houver planos
        updateWeeklyTrackerUI();
        updateProgressBarUI(); // Garante que a barra seja escondida
        showLoading(planLoadingViewDiv, false);
        return; // Sai da função se não há plano ativo
    }

    showLoading(planLoadingViewDiv, true);
    showErrorMessage(planViewErrorDiv, '');
    planCreationSection.style.display = 'none'; // Esconde criação ao carregar plano
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // *** VALIDAÇÃO CRUCIAL dos dados carregados ***
            if (!data || typeof data.plan !== 'object' || Array.isArray(data.plan) || data.plan === null ||
                ...
                typeof data.startDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(data.startDate) || // Verifica formato YYYY-MM-DD
                typeof data.endDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(data.endDate) ||   // Verifica formato YYYY-MM-DD
                !Array.isArray(data.allowedDays) ) {
                console.error("Invalid plan data format loaded:", data);
                // Tenta desativar o plano inválido
                currentReadingPlan = null;
                activePlanId = null;
            ...
            currentReadingPlan = { id: planId, ...data }; // Armazena com ID

            // Carrega interações semanais OU inicializa se for semana diferente/vazio
            ...
            loadDailyReadingUI(); // Atualiza a div de leitura diária (agora com data)
            updateWeeklyTrackerUI();
            updateProgressBarUI();
            readingPlanSection.style.display = 'block'; // Mostra a seção do plano
            planCreationSection.style.display = 'none'; // Garante que criação está oculta

        } else {
            console.warn("Active plan document (", planId, ") not found in Firestore.");
            currentReadingPlan = null;
            currentWeeklyInteractions = { weekId: currentWeekId, interactions: {} };
            // Desativa o plano no perfil do usuário se o documento não existe mais
            if(userInfo && userInfo.activePlanId === planId) {
                await updateDoc(doc(db, 'users', userId), { activePlanId: null });
                activePlanId = null; // Atualiza estado local também
            }
            readingPlanSection.style.display = 'none';
            planCreationSection.style.display = userPlansList.length === 0 ? 'block' : 'none'; // Mostra criação se não houver planos
            updateWeeklyTrackerUI();
            updateProgressBarUI();
            populatePlanSelector(); // Atualiza o seletor para refletir a desativação
        }
    } catch (error) {
...
/** Carrega todos os dados iniciais do usuário (info, lista de planos, plano ativo) */
async function loadUserDataAndPlans() {
    if (!currentUser) return;
    const userId = currentUser.uid;

    showLoading(planLoadingViewDiv, true); // Mostra loading geral
    readingPlanSection.style.display = 'none';
    planCreationSection.style.display = 'none';
    if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'none'; // Esconde upcoming
    showErrorMessage(planViewErrorDiv, ''); // Limpa erros antigos

    try {
        await fetchUserInfo(userId); // Busca info e activePlanId
        await fetchUserPlansList(userId); // Busca lista completa de planos
        populatePlanSelector(); // Popula o dropdown com a lista
        await loadActivePlanData(userId, activePlanId); // Tenta carregar o plano ativo (se houver)
        await displayUpcomingReadings(); // *** NOVO: Mostra as próximas leituras ***

    } catch (error) {
        console.error("Error during initial data load sequence:", error);
        // Em caso de erro grave, mostra opção de criar plano
        readingPlanSection.style.display = 'none';
        planCreationSection.style.display = 'block'; // Permite criar um novo plano
        if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'none';
    } finally {
        showLoading(planLoadingViewDiv, false);
    }
...
async function setActivePlan(planId) {
    ...
    if(planSelect) planSelect.disabled = true; // Desabilita durante a troca

    try {
        await updateDoc(userDocRef, { activePlanId: planId }); // Atualiza no Firestore
        activePlanId = planId; // Atualiza estado local
        if (userInfo) userInfo.activePlanId = planId; // Atualiza cache local de userInfo
        if (planSelect) planSelect.value = planId; // Seleciona no dropdown

        await loadActivePlanData(userId, planId); // Carrega os dados do novo plano ativo
        await displayUpcomingReadings(); // Atualiza as próximas leituras

        // Atualiza a lista no modal de gerenciamento se estiver aberto
        ...
        // Tenta reverter a seleção no dropdown para o plano anterior (se havia um)
        if (planSelect && currentReadingPlan) planSelect.value = currentReadingPlan.id;
        else if (planSelect) planSelect.value = ''; // Ou deixa vazio
    } finally {
        if(planSelect) planSelect.disabled = false; // Reabilita o seletor
    }
}
...
async function saveNewPlanToFirestore(userId, planData) {
    ...
    try {
        // Validações essenciais antes de salvar
        ...
        if (!Array.isArray(planData.allowedDays) /* Removido check de length > 0 aqui, permitido vazio para todos os dias */ ) throw new Error("Dias de leitura permitidos inválidos.");
        ...
        // Atualiza lista local imediatamente e define como ativo
...
} // Fechamento correto da função saveNewPlanToFirestore
...
async function updateProgressInFirestore(userId, planId, newDay, updatedWeeklyInteractions, logEntry = null) {
    ...
    if (markAsReadButton) markAsReadButton.disabled = true; // Desabilita botão durante salvar
    const planDocRef = doc(db, 'users', userId, 'plans', planId);
    ...
    try {
        ...
        // Adiciona entrada ao log se fornecida e válida
        if (logEntry && logEntry.date && /^\d{4}-\d{2}-\d{2}$/.test(logEntry.date) && Array.isArray(logEntry.chapters)) {
            // Usa notação de ponto para atualizar campo aninhado no log
        ...
        currentWeeklyInteractions = updatedWeeklyInteractions; // Atualiza cache local das interações
        if (logEntry && logEntry.date && Array.isArray(logEntry.chapters)) {
            if (!currentReadingPlan.readLog) currentReadingPlan.readLog = {};
            currentReadingPlan.readLog[logEntry.date] = logEntry.chapters; // Atualiza cache local do log
        }
        return true; // Sucesso
        ...
        // Reabilita botão (ou mantém desabilitado se plano concluído)
        ...
        if (markAsReadButton) {
            ...
            // Esconder o botão se o plano estiver completo
...
async function saveRecalculatedPlanToFirestore(userId, planId, updatedPlanData) {
    ...
    try {
        // Validação básica dos dados recalculados
        ...
        // Mantém dados originais importantes como createdAt e interações/log existentes
        const dataToSet = {
            ...currentReadingPlan, // Pega a base do plano atual (inclui readLog, weeklyInteractions, createdAt etc.)
            ...updatedPlanData     // Sobrescreve com os dados recalculados (plan, endDate, currentDay)
            // startDate é mantido do updatedPlanData (geralmente o mesmo do original)
        };

        await setDoc(planDocRef, dataToSet); // Usa setDoc para sobrescrever com os dados recalculados
        ...
        // Atualiza a lista local também, se encontrada
...
async function deletePlanFromFirestore(userId, planIdToDelete) {
    if (!userId || !planIdToDelete) {
        ...
        return false; // Não pode deletar sem IDs
    }
    ...
    try {
        ...
        if (activePlanId === planIdToDelete) {
            ...
            // Recarrega a UI para refletir o novo estado (ou estado sem plano ativo)
            populatePlanSelector();
            await loadActivePlanData(userId, activePlanId); // Carrega o novo ativo (ou nenhum)
            await displayUpcomingReadings(); // Atualiza próximas leituras

        } else {
            // Se o plano deletado NÃO era o ativo, apenas atualiza as listas visuais
            populatePlanSelector();
            if (managePlansModal.style.display === 'flex') {
                populateManagePlansModal(); // Atualiza o modal se estiver aberto
            }
            await displayUpcomingReadings(); // Atualiza próximas leituras pois um plano foi removido
        }
        ...
        // Mostra erro no modal de gerenciamento se estiver aberto, senão no erro geral do plano
...
function togglePlanCreationOptions() {
    ...
    // Visibilidade das opções de SELEÇÃO de capítulos
    ...
    // Visibilidade das opções de DURAÇÃO
    ...
    // Habilita/Desabilita inputs de duração
    ...
    // Desabilita opções de Duração se 'Capítulos por Dia' estiver selecionado
    if (durationMethodRadios) {
        durationMethodRadios.forEach(r => r.disabled = showChaptersPerDayOption);
    }

    // Esconde explicitamente opções de duração se 'Capítulos por Dia' for escolhido
    ...
    // Preenche data inicial padrão se opção de Data Final for selecionada e estiver vazia
    if (showEndDateOption && startDateInput && !startDateInput.value) {
        try {
            // Usa a data LOCAL do usuário como padrão para o input date
...
function showPlanCreationSection() {
    resetFormFields(); // Limpa o formulário
    readingPlanSection.style.display = 'none'; // Esconde leitura ativa
    if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'none'; // Esconde upcoming
    planCreationSection.style.display = 'block'; // Mostra criação
    if (cancelCreationButton) {
        // Mostra botão Cancelar apenas se já existem outros planos
...
function cancelPlanCreation() {
    ...
    if (currentReadingPlan && activePlanId) {
        readingPlanSection.style.display = 'block'; // Mostra leitura ativa
        if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'block'; // Mostra upcoming
    } else {
        // Se não há plano ativo, talvez não mostre nada ou volte para estado inicial
        // (loadUserDataAndPlans geralmente lida com isso, mostrando criação se lista vazia)
        console.log("Cancel creation: No active plan to return to.");
        // Poderia forçar recarga dos dados do usuário aqui se necessário
...
async function createReadingPlan() {
    ...
    // Não impede mais a criação se nenhum dia for selecionado (permitirá todos)
    ...
    try {
        if (creationMethod === 'interval') {
            ...
            if (!chaptersToRead) return; // Erro já mostrado por generateChaptersInRange
        ...
        let startDateStr = getCurrentUTCDateString(); // Padrão: hoje
        let totalReadingDays = 0; // Número de dias EFETIVOS de leitura
        let planMap = {};         // Mapa { '1': [...], '2': [...] }
        let endDateStr = '';      // Data final calculada
        ...
        if (creationMethod === 'chapters-per-day') {
            ...
            // Contar quantos dias de leitura existem dentro do período de calendário
            ...
            const inputStartDateStr = startDateInput.value;
            const inputEndDateStr = endDateInput.value;
            if (!inputStartDateStr || !inputEndDateStr) throw new Error("Selecione as datas de início e fim do calendário.");
            ...
            // Conta os dias de leitura no período
            let readingDaysInPeriod = 0;
        ...
        // A data final é a data correspondente ao ÚLTIMO dia de leitura no mapa
        ...
        const newPlanData = {
            name: planName,
            plan: planMap,               // Mapa de dias de leitura -> capítulos
            currentDay: 1,               // Começa no dia 1
            totalChapters: chaptersToRead.length, // Total de capítulos no plano
            chaptersList: chaptersToRead,       // Lista completa para referência/recálculo
            allowedDays: allowedDaysOfWeek,     // Dias da semana [0-6] (vazio significa todos)
            startDate: startDateStr,     // Data de início REAL (YYYY-MM-DD)
            endDate: endDateStr,         // Data de fim REAL (YYYY-MM-DD)
            readLog: {}                  // Log de leitura inicializado vazio
            // createdAt e weeklyInteractions serão adicionados em saveNewPlanToFirestore
        };

        //console.log("Dados do novo plano a serem salvos:", newPlanData);
...
/** Atualiza a UI para mostrar a leitura do dia atual (com data) */
function loadDailyReadingUI() {
    ...
    updateProgressBarUI(); // Atualiza barra de progresso primeiro
    ...
    if (!currentReadingPlan || !activePlanId) {
        ...
        readingPlanSection.style.display = 'none'; // Esconde seção principal do plano
        // Se não está na autenticação, mostra criação (se não houver planos)
    ...
    const { plan, currentDay, name, startDate, endDate, allowedDays } = currentReadingPlan;
    const totalReadingDaysInPlan = Object.keys(plan || {}).length;
    const isCompleted = currentDay > totalReadingDaysInPlan;

    // Atualiza título da seção com nome do plano
    ...
    if (isCompleted) {
        ...
        markAsReadButton.style.display = 'none'; // Esconde botão de marcar lido
        ...
        // Calcula a data REAL para o dia atual
        ...
        // Erro: dia inválido ou dados faltando (startDate, allowedDays)
...
async function markAsRead() {
    ...
    const userId = currentUser.uid;
    const { plan, currentDay, allowedDays, startDate } = currentReadingPlan; // Pega startDate e allowedDays
    const totalReadingDaysInPlan = Object.keys(plan || {}).length;

    // Só permite marcar se o dia atual é válido e o plano não está completo
    if (currentDay > 0 && currentDay <= totalReadingDaysInPlan && startDate && allowedDays) {
        const currentDayStr = currentDay.toString();
        const chaptersJustRead = plan[currentDayStr] || [];

        // **Importante:** O log usa a data UTC REAL de QUANDO foi marcado
        const actualDateMarkedStr = getCurrentUTCDateString();
        const currentWeekId = getUTCWeekId(); // Semana de QUANDO foi marcado

        // Calcula o PRÓXIMO dia de leitura no plano
        ...
        updatedWeeklyData.interactions[actualDateMarkedStr] = true; // Marca a data real

        // Cria a entrada de log
        const logEntry = {
            date: actualDateMarkedStr, // Data em que marcou
            chapters: chaptersJustRead // Capítulos correspondentes ao DIA DO PLANO que foi concluído
        };
        ...
        if (success) {
            ...
            loadDailyReadingUI(); // Recarrega a div de leitura (mostrará o novo dia/data)
            updateWeeklyTrackerUI(); // Atualiza marcador da semana
            await displayUpcomingReadings(); // Atualiza próximas leituras

            // Mensagem de conclusão se for o caso
        ...
        // Botão já é reabilitado/desabilitado dentro de updateProgressInFirestore

    } else {
        console.warn("Tentativa de marcar como lido quando plano já concluído, inválido ou dados faltando.", currentReadingPlan);
        // Garante que o botão esteja desabilitado se o plano estiver completo
...
function handleDeleteSpecificPlan(planIdToDelete) {
    ...
    if (confirm(`Tem certeza que deseja excluir o plano "${planName}" permanentemente? Todo o progresso e histórico serão perdidos.`)) {
        // Chama a função de exclusão do Firestore
        deletePlanFromFirestore(userId, planIdToDelete)
            .then(success => {
                if (success) {
                    alert(`Plano "${planName}" excluído com sucesso.`);
                    // Fecha o modal de gerenciamento se estiver aberto E a exclusão veio dele
                    if (managePlansModal.style.display === 'flex') {
                         closeModal('manage-plans-modal');
                    }
                    // UI já é atualizada dentro de deletePlanFromFirestore
                }
                // Mensagem de erro já é mostrada por deletePlanFromFirestore
            });
    }
}
...
function openModal(modalId) {
    ...
    if (modal) {
        ...
        if (paceInput) paceInput.value = '3'; // Padrão: 3 caps/dia
...
async function handleRecalculate() {
    ...
    // Pega uma cópia profunda dos dados relevantes do plano ATUAL
    const { chaptersList, currentDay, plan: originalPlanMap, startDate, allowedDays, name, readLog, weeklyInteractions, createdAt } = JSON.parse(JSON.stringify(currentReadingPlan));
    ...
    try {
        // 1. Determinar capítulos restantes
        let chaptersReadCount = 0;
        // Conta capítulos em dias ANTERIORES ao dia atual
        for (let dayKey in originalPlanMap) {
            const dayNum = parseInt(dayKey, 10);
            // Considera apenas os dias estritamente menores que o atual
            if (dayNum < currentDay && Array.isArray(originalPlanMap[dayKey])) {
                chaptersReadCount += originalPlanMap[dayKey].length;
            }
        }
        // Garante que não contamos mais capítulos do que existem
        ...
        // 2. Calcular nova distribuição e duração
        let newTotalReadingDays = 0; // Total de dias de leitura APÓS o recálculo
        let newPlanMap = {};         // Novo mapa APENAS para os dias restantes

        if (recalcOption === 'extend_date') {
            // Mantém o ritmo médio original (calculado sobre dias de LEITURA)
            ...
            const avgPace = originalReadingDaysWithContent > 0
                ? Math.max(1, Math.ceil(chaptersList.length / originalReadingDaysWithContent)) // Usa total de caps / total dias leitura originais
                : 3; // Fallback
            ...
            // Mantém a data final -> calcula quantos dias de leitura RESTAM até lá
            const originalEndDate = new Date(currentReadingPlan.endDate + 'T00:00:00Z'); // Usa endDate armazenado
            ...
            let currentDate = new Date(todayDate); // Começa de hoje

            // Calcula data correspondente ao dia atual do plano para iniciar a contagem
            ...
            if(currentDateOfPlanDayStr){
                ...
                console.warn("Não foi possível calcular a data do dia atual do plano, usando hoje como fallback para cálculo do recálculo.");
            }


            // Conta dias de leitura permitidos de 'currentDate' até 'originalEndDate' (inclusive)
            while (currentDate <= originalEndDate) {
                ...
                currentDate.setUTCDate(currentDate.getUTCDate() + 1);
                 // Safety break
                 if(remainingReadingDaysCount > 365*5) break;
            }

            newTotalReadingDays = Math.max(1, remainingReadingDaysCount); // Pelo menos 1 dia
            newPlanMap = distributeChaptersOverReadingDays(remainingChapters, newTotalReadingDays);
        ...
        if (Object.keys(newPlanMap).length === 0 && remainingChapters.length > 0) {
            throw new Error("Falha ao redistribuir os capítulos restantes (mapa vazio gerado).");
        }

        // 3. Montar o mapa completo do plano atualizado
        const updatedFullPlanMap = {};
        // Copia os dias ANTERIORES ao dia atual do mapa original
        for (let dayKey in originalPlanMap) {
            const dayNum = parseInt(dayKey, 10);
        ...
        // Adiciona os dias RECALCULADOS, começando do 'currentDay'
        let newMapDayCounter = 0;
        Object.keys(newPlanMap).sort((a,b) => parseInt(a) - parseInt(b)).forEach(remDayKey => {
            const newDayKey = (currentDay + newMapDayCounter).toString(); // Chave numérica sequencial
            updatedFullPlanMap[newDayKey] = newPlanMap[remDayKey]; // Associa capítulos do mapa recalculado
            newMapDayCounter++;
        });

        // 4. Calcular a nova data final REAL
        ...
        // 5. Preparar dados para salvar (sobrescrever)
        const updatedPlanData = {
            // Dados que NÃO mudam com recálculo simples
            name: name,
            ...
            createdAt: createdAt || serverTimestamp(), // Mantém timestamp original
            // Dados ATUALIZADOS
            plan: updatedFullPlanMap,       // O novo mapa distribuído
            currentDay: currentDay,         // Continua do dia atual
            startDate: startDate,         // Data de início NÃO muda
            endDate: newEndDateStr          // A NOVA data final calculada
        };

        //console.log("Dados recalculados a serem salvos:", updatedPlanData);
        ...
        if (success) {
            ...
            loadDailyReadingUI(); // Atualiza a exibição do dia
            updateProgressBarUI(); // Atualiza a barra de progresso com nova data final
            await displayUpcomingReadings(); // Atualiza próximas leituras
            populatePlanSelector(); // Atualiza o dropdown com a nova data final
        }
...
function displayReadingHistory() {
    ...
    showLoading(historyLoadingDiv, false); // Esconde loading (assume que dados já estão em currentReadingPlan)
    showErrorMessage(historyErrorDiv, ''); // Limpa erros
    ...
    const sortedDates = Object.keys(readLog)
                          .filter(dateStr => /^\d{4}-\d{2}-\d{2}$/.test(dateStr)) // Filtra chaves válidas
                          .sort() // Ordena por data (ascendente)
    ...
    // Cria elementos para cada entrada do log
    sortedDates.forEach(dateStr => {
        const chaptersRead = readLog[dateStr] || [];
        const entryDiv = document.createElement('div');
        entryDiv.classList.add('history-entry');

        const formattedDate = formatUTCDateStringToBrasilian(dateStr); // Formata data
        const chaptersText = chaptersRead.length > 0
                            ? chaptersRead.join(', ')
                            : 'Nenhum capítulo registrado neste dia.'; // Mensagem se array vazio
...
async function calculateAndShowStats() {
    ...
    //const userId = currentUser.uid; // Não usado diretamente aqui, mas pode ser útil para stats globais futuras
    ...
    try {
        ...
        if (currentReadingPlan && activePlanId) {
            ...
            if (activePlanTotalReadingDays > 0) {
                ...
                if (currentReadingPlan.currentDay > activePlanTotalReadingDays) activePlanProgress = 100; // Garante 100% se concluído
            }

            // Calcula caps lidos e dias lidos a partir do readLog do plano ativo
            const readLog = currentReadingPlan.readLog || {};
            Object.values(readLog).forEach(chaptersArray => {
                if (Array.isArray(chaptersArray)) {
                    activePlanChaptersRead += chaptersArray.length;
                    activePlanDaysRead++; // Conta um dia lido para cada entrada no log
                }
            });
        }
        ...
        // 2. Stats Gerais (Simplificadas por enquanto - baseadas no PLANO ATIVO)
        //    -> Futuramente, poderia iterar userPlansList para totais reais
        statsTotalChapters.textContent = activePlanChaptersRead > 0 ? activePlanChaptersRead : "--"; // Total lido NO PLANO ATIVO
...
// --- *** NOVO: Função para Próximas Leituras *** ---
/**
 * Busca e exibe as próximas N leituras agendadas em todos os planos do usuário.
 * @param {number} count - Quantas próximas leituras exibir.
 */
async function displayUpcomingReadings(count = 3) {
    if (!upcomingReadingsSection || !upcomingReadingsListDiv || !currentUser) {
        console.warn("Elementos UI para 'Upcoming Readings' ou usuário não disponíveis.");
        if (upcomingReadingsSection) upcomingReadingsSection.style.display = 'none';
        return;
    }

    showLoading(upcomingReadingsLoadingDiv, true);
    upcomingReadingsSection.style.display = 'block'; // Mostra a seção
    upcomingReadingsListDiv.innerHTML = ''; // Limpa conteúdo anterior
    showErrorMessage(planViewErrorDiv, ''); // Limpa erros gerais que podem ter ficado
    ...
    try {
        ...
        for (const plan of userPlansList) {
            // Valida dados essenciais do plano para cálculo
            if (!plan.id || !plan.plan || typeof plan.currentDay !== 'number' || !plan.startDate || !plan.allowedDays || typeof plan.plan !== 'object') {
                console.warn(`Plano ${plan.id || 'desconhecido'} (${plan.name || 'sem nome'}) pulado por falta de dados para cálculo de datas.`);
                continue; // Pula plano inválido
            }
            ...
            // Calcula as próximas 'count' datas de leitura para ESTE plano
            let readingDaysFoundForThisPlan = 0;
            for (let dayOffset = 0; readingDaysFoundForThisPlan < count; dayOffset++) {
                ...
                if (dateStr) {
                    ...
                    // Adiciona à lista apenas se houver capítulos designados (ou se for um dia de 'descanso' planejado - opcional)
                     if (chaptersForDay.length > 0) {
                        ...
                        readingDaysFoundForThisPlan++;
                    }
                     // Se quiser mostrar dias sem leitura:
                     /* else {
                         upcomingList.push({ date: dateStr, planId: plan.id, planName: plan.name, chapters: "Dia sem leitura designada" });
                         readingDaysFoundForThisPlan++;
                     } */
                } else {
                     // Se calculateDateForDay retornar null, pode indicar fim do loop de segurança ou erro
                     console.warn(`Não foi possível calcular data para dia ${targetDayNumber} do plano ${plan.id}. Parando busca para este plano.`);
                     break; // Para a busca para este plano se houver erro
                }

                 // Safety break interno para evitar loops muito longos por plano
                 if (dayOffset > totalReadingDays + 7) { // Permite buscar um pouco além do fim teórico
                     console.warn(`Safety break atingido ao buscar próximas leituras para plano ${plan.id}`);
                     break;
                 }

            } // Fim do loop for dayOffset
        ...
        // Exibe as 'count' primeiras entradas
        if (upcomingList.length === 0) {
            upcomingReadingsListDiv.innerHTML = '<p>Nenhuma leitura próxima agendada nos seus planos ativos.</p>';
        } else {
            const itemsToShow = upcomingList.slice(0, count);
            itemsToShow.forEach(item => {
                const itemDiv = document.createElement('div');
                itemDiv.classList.add('upcoming-reading-item'); // Adicionar estilo CSS para esta classe
                itemDiv.innerHTML = `
                ...
                `;
                // Opcional: Adicionar listener para ativar o plano ao clicar
                itemDiv.addEventListener('click', () => {
                    if (item.planId !== activePlanId) {
                        setActivePlan(item.planId);
                        // Rolar para a seção do plano ativo?
                        // readingPlanSection.scrollIntoView({ behavior: 'smooth' });
                    }
                });
                itemDiv.style.cursor = 'pointer'; // Indica clicável
        ...
        showErrorMessage(planViewErrorDiv, `Erro nas próximas leituras: ${error.message}`); // Mostra erro geral também
    } finally {
        showLoading(upcomingReadingsLoadingDiv, false);
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!loginButton || !createPlanButton || !markAsReadButton || !recalculateModal || !managePlansModal || !statsModal || !historyModal || !planSelect || !periodicityCheckboxes || !upcomingReadingsSection || !upcomingReadingsListDiv ) {
        console.error("Erro crítico: Elementos essenciais da UI (incluindo 'Upcoming Readings') não encontrados.");
    ...
    populateBookSelectors(); // Popula dropdowns de livros
    togglePlanCreationOptions(); // Ajusta visibilidade inicial das opções
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showErrorMessage(authErrorDiv, `Erro de login: ${error.code} - ${error.message}`); // Mostra código e msg
        } finally {
    ...
    signupForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Limpa campos após sucesso (auth state change vai lidar com o login)
             alert("Cadastro realizado com sucesso! Você já está logado.");
             if (signupEmailInput) signupEmailInput.value = '';
             if (signupPasswordInput) signupPasswordInput.value = '';
        } catch (error) {
            console.error("Signup error:", error);
            showErrorMessage(signupErrorDiv, `Erro no cadastro: ${error.code} - ${error.message}`); // Mostra código e msg
        } finally {
    ...
    logoutButton.addEventListener('click', async () => {
        ...
        // finally { logoutButton.disabled = false; } // O observer onAuthStateChanged vai reabilitar indiretamente
    });
    ...
    // Listeners para atualizar opções dinamicamente
    ...
    // Listener para sugestões de livros
    if (chaptersInput) chaptersInput.addEventListener('input', updateBookSuggestions);
     ...
     deleteCurrentPlanButton.addEventListener('click', () => {
         if(activePlanId && currentReadingPlan) {
             handleDeleteSpecificPlan(activePlanId); // Chama wrapper com confirmação
         } else {
     ...
     showStatsButton.addEventListener('click', () => {
         calculateAndShowStats(); // Calcula stats antes de abrir
         openModal('stats-modal');
     });
     showHistoryButton.addEventListener('click', () => {
         displayReadingHistory(); // Popula histórico antes de abrir
     ...
     planSelect.addEventListener('change', (e) => {
         ...
         if (selectedPlanId && selectedPlanId !== activePlanId) {
             setActivePlan(selectedPlanId); // Ativa o plano selecionado
         }
     });
     managePlansButton.addEventListener('click', () => {
         populateManagePlansModal(); // Popula o modal antes de abrir
     ...
     createNewPlanButton.addEventListener('click', () => {
         closeModal('manage-plans-modal');
         showPlanCreationSection(); // Mostra seção de criação
     });
     ...
     // Garantir que botões de fechar (X) funcionem (já estão com onclick no HTML)

    // --- Firebase Auth State Observer ---
    onAuthStateChanged(auth, (user) => {
        //console.log("Auth state changed. User:", user ? user.uid : "null");
        // Reabilita botões de auth que podem ter ficado desabilitados por erros anteriores
        if (loginButton) loginButton.disabled = false;
        if (signupButton) signupButton.disabled = false;
        if (logoutButton) logoutButton.disabled = false; // Será desabilitado se não houver user
        // Atualiza toda a UI com base no novo estado de autenticação
...
// Expor closeModal globalmente para os 'onclick' no HTML
