...
function initApplication() {
    ...
    // --- INÍCIO DA ALTERAÇÃO (CORREÇÃO ESSENCIAL) ---
    modalsUI.init({
        onConfirmRecalculate: async (option, newPace) => {
            const confirmBtn = document.getElementById('confirm-recalculate');
            const planId = confirmBtn.dataset.planId;
            if (!planId) {
                // Se por algum motivo o planId não estiver presente, exibe um erro claro.
                modalsUI.showError('recalculate-modal', 'Erro crítico: ID do plano não encontrado. Tente reabrir esta janela.');
                return;
            }
            
            // O bloco try...catch aqui é crucial. Ele "escuta" por qualquer erro
            // que possa ocorrer dentro da função assíncrona handleRecalculate.
            try {
                // Chamamos a função principal e esperamos (await) sua conclusão.
                await handleRecalculate(option, newPace, planId);
            } catch (error) {
                // Se qualquer erro for lançado de handleRecalculate, ele será pego aqui
                // e exibido na interface do usuário.
                console.error("Falha ao confirmar o recálculo:", error);
                modalsUI.showError('recalculate-modal', `Falha na operação: ${error.message}`);
                modalsUI.hideLoading('recalculate-modal'); // Garante que o loading seja escondido em caso de erro.
            }
        },
    });
    // --- FIM DA ALTERAÇÃO ---
