...
export function displayMyHighlights(plan) {
    // Nova linha de segurança: previne erros no console caso o HTML não carregue
    if (!myHighlightsList) {
    ...
    for (const [chapter, versesArray] of Object.entries(highlights)) {
        if (versesArray.length === 0) continue;
        ...
        versesArray.forEach((v, idx) => {
            const isDone = v.done ? 'checked' : '';
            const doneClass = v.done ? 'done' : '';
            html += `<div class="history-verse-item ${doneClass}">
                        <input type="checkbox" class="verse-highlight-check" data-plan-id="${plan.id}" data-chapter="${chapter}" data-verse-index="${idx}" ${isDone}>
